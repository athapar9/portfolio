        ...
        <section className="w-full">
          <div className="flex flex-col w-full mb-6">
            <div className="flex items-center w-full">
              <h1 className="text-3xl font-serif mr-4 text-[var(--accent)]">
                Projects
              </h1>
              <div className="flex-grow border-t border-gray-300" />
            </div>
            <p className="mt-3 italic text-foreground-500 max-w-full text-center">
          ...
          <div className="space-y-8">
            {projects.map(({ title, tech, points, github }) => (
              <div
                key={title}
                className="p-6 rounded-xl border border-gray-300 bg-white/5 hover:bg-white/10 transition"
              >
                <h2 className="font-serif font-semibold text-xl mb-2">
                  {title}
                </h2>
                <p className="italic text-muted-foreground mb-4">{tech}</p>
                <ul className="list-disc list-inside space-y-2 mb-4 text-lg font-serif leading-relaxed">
                  {points.map((point, i) => (
                    <li key={i}>{point}</li>
                  ))}
                </ul>
                <a
                  ...
                  className="inline-flex items-center px-4 py-2 border border-gray-600 rounded-md hover:bg-[var(--accent)] transition"
          ...
          <div className="border-b border-gray-300 w-full mt-10" />
        </section>
      </main>
    </div>
  );
